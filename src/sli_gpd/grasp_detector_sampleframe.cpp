...
GraspDetector::GraspDetector(ros::NodeHandle& node)
{
  ...
  nn_radius_frames_=hand_search_params.nn_radius_frames_;
  ...
  frame_estimator_= new FrameEstimator(hand_search_params.num_threads_);
...
std::vector<Grasp> GraspDetector::detectGrasps(const CloudCamera& cloud_cam)
{
  ...
  CloudCamera keep_cloud_cam=cloud_cam;
  ...
  std::vector<Grasp> valid_grasps = GMM_classifyGraspCandidates(cloud_cam, candidates,keep_cloud_cam);
  ...
  {
    sum = sum + clustered_grasps[i].getScore();
  }
  double score_mean= sum / (double) clustered_grasps.size();
...
std::vector<Grasp> GraspDetector::GMM_classifyGraspCandidates(const CloudCamera& cloud_cam,
  std::vector<GraspSet>& candidates, const CloudCamera& keep_cloud_cam)
{
  ...
  for (int i=0;i<num_iters;i++)
  {
    ...
    const PointCloudRGB::Ptr& cloud = keep_cloud_cam.getCloudProcessed();
    ...
    for (auto& g : elite_grasps){
      ...
      Eigen::MatrixXd v(v_bottom.rows(),v_bottom.cols()+v_surface.cols()+v_sample.cols());
      v << v_bottom, v_surface, v_sample;
      Eigen::MatrixXd frame_(3,3);
      Eigen::MatrixXd v_frame_(3,3);
      v_frame_=g.getFrame();
      LocalFrame* frame = frame_estimator_->calculateFrame(keep_cloud_cam.getNormals(), v_sample, nn_radius_frames_, kdtree);
      frame_ << frame->getNormal(), frame->getBinormal(), frame->getCurvatureAxis();
      std::cout << "g getframe_:"<<v_frame_<< std::endl;
      std::cout << "v_sample:"<<v_sample<< std::endl;
      std::cout << "frame_:"<<frame_<< std::endl;
      std::cout << "v_sample:"<<frame.getSample()<< std::endl;
    ...
    // bool status = gmm_model.learn(elite_grasps_arr, clustered_grasps.size(), arma::maha_dist, arma::random_subset, 10, 8, 0.01, true);
    // if (!status)
    //  ROS_ERROR("GaussianMixture learn");
    // int num_gmm_samples=200;
    ...
    std::vector<Grasp> gmm_candidates=from_feature_vec(keep_cloud_cam,grasp_vecs_mat,nn_radius_frames_);

    std::cout << "gmm_candidates.getFrame():"<< gmm_candidates[0].getFrame()<< std::endl;
    std::cout << "gmm_candidates.getGraspBottom():"<<gmm_candidates[0].getGraspBottom() <<std::endl;
    std::cout << "gmm_candidates.getSample():"<< gmm_candidates[0].getSample()<< std::endl;
    std::cout << "gmm_candidates.getFrame():"<< gmm_candidates[0].getFrame()<< std::endl;
    std::cout << "gmm_candidates.getGraspBottom():"<<gmm_candidates[1].getGraspBottom() <<std::endl;
    std::cout << "gmm_candidates.getSample():"<< gmm_candidates[1].getSample()<< std::endl;
    std::cout << "gmm_candidates.getFrame():"<< gmm_candidates[1].getFrame()<< std::endl;
    std::cout << "gmm_candidates.getGraspBottom():"<<gmm_candidates[2].getGraspBottom() <<std::endl;
    std::cout << "gmm_candidates.getSample():"<< gmm_candidates[2].getSample()<< std::endl;
    std::cout << "gmm_candidates.getFrame():"<< gmm_candidates[2].getFrame()<< std::endl;
    // if (plot_valid_grasps_)
    // {
    //   Plot plotter;
    //   const HandSearch::Parameters& params = candidates_generator_->getHandSearchParams();
...
std::vector<Grasp> GraspDetector::from_feature_vec( const CloudCamera& cloud_cam, arma::mat& v, double nn_radius_frames_)
{
  ...
  for (int i=0; i<v.n_cols;i++)
  {
    ...
    grasp_mat=eigen_v.block<9,1>(0,i);
    grasp_mat.resize(3,3);
    Grasp vector_grasp;
    Eigen::Matrix3Xd grasp_sample;
    grasp_sample.resize(3,0);
    grasp_sample=grasp_mat.block<3,1>(0,2);
    vector_grasp.pose_.bottom_=grasp_mat.block<3,1>(0,0);
    vector_grasp.pose_.surface_=grasp_mat.block<3,1>(0,1);
    vector_grasp.sample_=grasp_mat.block<3,1>(0,2);
    //ROS_INFO_STREAM("vector_grasp.sample_"<<vector_grasp.sample_);
    const PointCloudRGB::Ptr& cloud = cloud_cam.getCloudProcessed();
    pcl::KdTreeFLANN<pcl::PointXYZRGBA> kdtree;
    kdtree.setInputCloud(cloud);

    LocalFrame* frame = frame_estimator_->calculateFrame(cloud_cam.getNormals(), vector_grasp.sample_, nn_radius_frames_, kdtree);
    vector_grasp.pose_.frame_ << frame->getNormal(), frame->getBinormal(), frame->getCurvatureAxis();

    //ROS_INFO_STREAM("vector_grasp.pose_.frame_"<<vector_grasp.pose_.frame_);
