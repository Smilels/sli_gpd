...
GraspDetector::GraspDetector(ros::NodeHandle& node)
{
  ...
  node.param("use_gmm", use_gmm_, true);
}
...
std::vector<Grasp> GraspDetector::detectGrasps(const CloudCamera& cloud_cam)
{
  ...
  std::vector<double> gmm_score_mean;
  std::vector<std::vector<Grasp>> se;
  gmm_grasps = GMM_classifyGraspCandidates(cloud_cam, candidates);
  for (int j=0;j< gmm_grasps.size(); j++)
  {
    std::vector<Grasp> gmm_valid_grasps;
    gmm_valid_grasps=gmm_grasps[j];
    ...
    double score_mean= gmm_sum / (double) gmm_valid_grasps.size();
    gmm_score_mean.push_back(score_mean);
    std::cout << "gmm_score_mean: " << score_mean << "\n";

    std::vector<Grasp> selected_grasps1;
    // 5. Select highest-scoring grasps.
    if (gmm_valid_grasps.size() > num_selected_)
    //if (clustered_grasps.size() > 1000)
    {
      std::cout << "Partial Sorting the grasps based on their score ... \n";
      std::partial_sort(gmm_valid_grasps.begin(),gmm_valid_grasps.begin() + num_selected_, gmm_valid_grasps.end(),
        isScoreGreater);
      selected_grasps1.assign(gmm_valid_grasps.begin(), gmm_valid_grasps.begin() + num_selected_);
    }
    else
    {
      std::cout << "Sorting the grasps based on their score ... \n";
      std::sort(gmm_valid_grasps.begin(),gmm_valid_grasps.end(), isScoreGreater);
      selected_grasps1 = gmm_valid_grasps;
    ...
    se.push_back(selected_grasps1);
  }
  auto max_mean=max_element(gmm_score_mean.begin(), gmm_score_mean.end());
  std::cout << "max_mean " << std::distance(std::begin(gmm_score_mean), max_mean) << "\n";
  return se[std::distance(std::begin(gmm_score_mean), max_mean)];
}
...
std::vector<std::vector<Grasp>> GraspDetector::GMM_classifyGraspCandidates(const CloudCamera& cloud_cam,
  std::vector<GraspSet>& candidates)
{
  Eigen::VectorXd coe(6);
  coe << 0.05,0.1,0.15,0.2,0.25,0.3;
  std::vector<std::vector<Grasp>> grasps_end;
  for (int n;n<coe.size(); n++)
{
  ...
  for (int i=0;i<num_iters;i++)
  {
    ...
    int num_gmm=std::ceil(coe(n) * num_refit);
    num_gmm =std::max(num_gmm,1);
    bool status = gmm_model.learn(elite_grasps_arr,num_gmm, arma::maha_dist, arma::random_subset, 10, 8, 0.01, true);
    if (!status)
  ...
  grasps_end.push_back(valid_grasps_end);
}
  return grasps_end;
}
